...
newhighscore = pygame.mixer.Sound(path.join(sound_folder, 'newhighscore.wav'))
...
def enterbox(screen, txt):
    ...
    def show_name(screen, name):
        ...
        txt_surf = font.render(name, True, GREY)
    ...
    box.fill(BLACK)
    pygame.draw.rect(box, BLACK, (0, 0, bx, by), 1)
    txt_surf = font.render(txt, True, GREY)
...
def highscore(screen, file_name, your_points):
    ...
    if your_points > high_score:
        ...
        your_name = enterbox(screen, "YOU HAVE THE SAME HIGHSCORE AS - What is your name?")
